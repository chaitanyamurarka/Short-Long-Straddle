...
from virtual import long_straddle
# from real import long_straddle
login = pd.read_csv('long_login.csv')
    ...
    try:
        if len(last_access_token)>=10:
            ...
            for i in kite.instruments():
                if (i['name'] in eval(row['long straddle']).keys()):
