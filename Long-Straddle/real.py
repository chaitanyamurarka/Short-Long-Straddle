...
def long_get_symbol_lotsize(instruments,name,last_thursday_date_dt,kite,client):
    IST = pytz.timezone('Asia/Kolkata')
    logging.info(f'{datetime.now(IST)} For {client} Scanning Entry Long Straddle Option Chain of {name}')
...
def check_rentry_long_straddle(existing_positions,name,client):
    data = pd.read_csv('long_login.csv')
    ...
    if (
        ...
        if check_rentry_long_straddle(existing_positions,name,client):
            if long_net_quant_zero(existing_positions,name):
                tradingsymbol_ce,lot_size_ce,tradingsymbol_pe,lot_size_pe ,instru_ce,instru_pe = long_get_symbol_lotsize(instruments,name,last_thursday_date_dt,kite,client)
                if (tradingsymbol_ce is not None and lot_size_ce is not None and tradingsymbol_pe is not None and lot_size_pe is not None):
                    logging.info(f'{datetime.now(IST)} For {client} ENTERING Long STRADDLE FOR {val} lots\n{tradingsymbol_ce} OF LOT SIZE {lot_size_ce} \nand\n{tradingsymbol_pe} of LOT SIZE {lot_size_pe}')
    ...
    if datetime.now(IST).time() >= datetime.strptime('09:25', '%H:%M').time():
        for position in existing_positions:
            if (get_name_from_instrument_token(instruments,position['instrument_token']) == name 
                ...
                buy_ce = get_sell_ce(kite,position['tradingsymbol'])
                buy_pe = get_sell_pe_from_ce(kite,trad_pe)
                ltp_ce = ((kite.quote(int(instru_ce)))[str(instru_ce)])['last_price']
                ltp_pe = ((kite.quote(int(instru_pe)))[str(instru_pe)])['last_price']
                logging.info(f"{datetime.now(IST)} For {client} Checking Long Exit Condtion for {name} with current CE ltp {ltp_ce} & PE ltp {ltp_pe}")
                if (
                    (ltp_pe + ltp_ce) / (buy_ce + buy_pe) <= 0.8
                or
                (
                    (ltp_pe + ltp_ce) / (buy_ce + buy_pe) >= 1.2
                )
                ):
